...
library(magrittr)
cdir <- getwd()
cdurl <- paste("file://",cdir,"/pkgs/flights.tar.gz",sep="")
suppressMessages(install_url(cdurl))
suppressMessages(library(flights))
...
month <- opt_get("month",required=TRUE,description="Month 1-12") %>% as.numeric()
day_of_week <- opt_get("day_of_week",required=TRUE,description="Day Of Week") %>% as.numeric()
